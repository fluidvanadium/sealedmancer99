...
async fn app() -> Result<u64, Error> {
    ...
    let query = Query::Custom(
        "(legal:vintage -t:stickers -o:sticker -o:ticket -o:{TK} (-t:attraction -o:Attraction or name:attraction) not:meld -o:draft -t:basic or (fo:meld)) or (name:/^a-/) or 'Stone-Throwing Devils' or 'Pradesh Gypsies' or 'Shahrazad' or 'Downdraft' or 'Backdraft'".to_string(),
    );
